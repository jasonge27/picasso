sqrtlasso_solver <- function(Y, X, lambda, nlambda, gamma, n, d, max.ite, prec, verbose, method.flag)
{
  if (verbose){
    if (method.flag == 1)
  ...
  for (i in 1:nlambda){
    beta.i = unlist(str[11])[((i-1)*d+1):(i*d)]
    beta.list[[i]] = beta.i
  }
  beta.intcpt = unlist(str[12])
  ite.lamb = unlist(str[13])
  size.act = unlist(str[14])
  runt = matrix(unlist(str[15]), ncol = nlambda, byrow = FALSE)
